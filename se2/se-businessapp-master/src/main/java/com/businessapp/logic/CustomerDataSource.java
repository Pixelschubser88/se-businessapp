...
   */
                persistenceProvider.loadInto(customers.getId(),entity -> {this.customers.store((Customer) entity);return true;
});
            } catch (IOException e) {
                System.out.print(", ");
    ...
    @Override
    public Customer newCustomer(String firstname, String name) {
        Customer customer = new Customer(null,firstname,name);
        customers.store(customer);
        if( persistenceProvider != null) persistenceProvider.save(customers, customers.getId());
        return customer;
    }
